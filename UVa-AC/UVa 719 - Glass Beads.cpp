/*
  ...
  Date: 1/4/2013
  Algorithm: suffix array, lexicographically order of rotations
  Time Limit: 3.000s
*/
...
int t, n, SA[MAXN], RA[MAXN], tmpSA[MAXN], tmpRA[MAXN], c[MAXN], Phi[MAXN], PLCP[MAXN], LCP[MAXN];
char T[MAXN], line[MAXN];
...
void solve() {
    ...
    while(pos < n - 1 && LCP[pos + 1] >= n / 2) pos++;
    printf("%d\n", SA[pos] % (n / 2) + 1);
}
