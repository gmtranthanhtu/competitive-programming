/*
    Name: ChessMaster
    Version: 4.0
    Date: 3:26, 10 June 2013
    Creator: 3T
    Description: Based on ChessMaster v3.0.
    Update: dijsktra to neareast available gold and next respawning gold, check availalbe gold
            - update respawning enemies
            - if there's no good moves, try to escape from enemy
*/
...
*/
    ...
    rep(i, FIELD_ROW_COUNT) rep(j, FIELD_COLUMN_COUNT) {
        ...
        res = min(res, max(d[i][j], goldActive[i][j] + 1));
    }
    ...
    if ( myRow >= 0 && myCol >= 0 ) {
      ...
      if ( canDigLeft && approachingLeft && threat[LEFT]) {
        ...
        if(canDigRight && !isAround(myRow, myCol, enemies) && !fallIntoEnemy(myRow + 1, myCol + 1)) {
            int tmp2 = countNearbyEnemy(myRow + 1, myCol + 1, enemies);
            int tmp1 = getMinDistanceToGold(myRow + 1, myCol + 1, enemies, 6) + 3;
        ...
        if(!isAround(myRow, myCol, enemies)){
            ...
            int tmp1 = getMinDistanceToGold(myRow, myCol, enemies, 0) + 1;
        ...
        if(moves.size() == 0) {
            if(approachingLeft) move = "RIGHT";
            else if(approachingRight) move = "LEFT";
        }
