/*
    Name: ChessMaster
    Version: 6.0
    Date: 3:34, 14 June 2013
    Creator: 3T
    Description: Based on ChessMaster v5.0.
    Update: nothing much
*/
...
*/
      ...
      // 11/6/2013: try to trap opponent
        ...
        for ( int i = 1; i <= 4; i++ )
          if ( canMove[ i ] && !threat[ i ] && !isAround(myRow + dRow[i], myCol + dCol[i]) && !fallIntoNoEscape(myRow + dRow[i], myCol + dCol[i])) {
