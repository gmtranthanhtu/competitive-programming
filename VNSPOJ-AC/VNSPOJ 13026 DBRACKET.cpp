/*
  Name: VNSPOJ DBRACKET
  Author: 3T - mailto: gm.tranthanhtu@gmail.com
  Date: 5/4/2013
  Algorithm: suffix array, lcp, distinct substrings, binary search
  Time Limit: 15.000s
*/
...
int t, n, sa[MAXN], ra[MAXN], tmpra[MAXN], tmpsa[MAXN], p[MAXN], lcp[MAXN], phi[MAXN], c[MAXN];
int cp, curd, b[MAXN], pos[MAXN], Next[MAXN];
ii d[MAXN];
char T[MAXN];
vi S[MAXN];
...
int solve () {
    int res = 0;
    fr(i, 1, n - 1) {
        if(i == 1) {res += b[sa[1]]; continue;}
        if(T[sa[i]] == ')') break;
        if(b[sa[i]] == 0) continue;
        int tmp = sa[i] + lcp[i] - 1;
        int m = d[sa[i]].ff, hi = d[sa[i]].ss, lo = 0, mi;
        int tmpres = -1;
        while(lo <= hi) {
            mi = (lo + hi) / 2;
            if(Next[S[m][mi]] > tmp) {
